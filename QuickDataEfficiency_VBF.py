...
def thresholds(trig):
    thrshlds = {    #pt1, pt2, mjj, detajj
        'VBFincl' : [105, 40, 1000, 3.5],
        'VBFjets' : [70,  40,  600, 2.5],
        'VBFmet'  : [75,  40,  500, 2.5],
        'VBFphot' : [45,  45,  500, 2.5],
        'VBFele'  : [45,  45,  500, 2.5],
        'VBFmu'   : [90,  40,  600, 2.5],
        'VBFtau'  : [45,  45,  500, 2.5],
    }
    return thrshlds[trig]
...
def get_path_dict(year):
    if '23' in year:
        ...
        print("VBF paths only available for 2023!")
        return {}
...
class TrigBtagAnalysis(Module):
    ...
    def beginJob(self,histFile=None,histDirName=None):
        ...
        # eff integrated
        self.h_tot  = ROOT.TH1F("h_tot" , "; passed ref trigger" , 1 , 0. , 2. )
        self.h_pass = ROOT.TH1F("h_pass" , "; passed ref trigger" , 1 , 0. , 2. )
        self.addObject(self.h_tot)
        self.addObject(self.h_pass)
        # eff vs NPV
        self.h_npv_tot  = ROOT.TH1F("h_npv_tot" ,   "; NPV" , 100,  0., 100.  )
        self.h_npv_pass = ROOT.TH1F("h_npv_pass" ,  "; NPV" , 100,  0., 100.  )
        self.addObject(self.h_npv_tot)
        self.addObject(self.h_npv_pass)     
        # eff vs pTj1
        self.h_pt1_tot  = ROOT.TH1F("h_pt1_tot" ,  "; Leading jet pT" , 1000, 0., 1000. )
        self.h_pt1_pass = ROOT.TH1F("h_pt1_pass" , "; Leading jet pT" , 1000, 0., 1000. )
        self.addObject(self.h_pt1_tot)
        self.addObject(self.h_pt1_pass)
        # eff vs pTj2
        self.h_pt2_tot  = ROOT.TH1F("h_pt2_tot" ,  "; subLeading jet pT" , 1000, 0., 1000. )
        self.h_pt2_pass = ROOT.TH1F("h_pt2_pass" , "; subLeading jet pT" , 1000, 0., 1000. )
        self.addObject(self.h_pt2_tot)
        self.addObject(self.h_pt2_pass)
        # eff vs mjj
        self.h_mjj_tot  = ROOT.TH1F("h_mjj_tot" ,   "; mjj" ,     1000, 0., 2000. )
        self.h_mjj_pass = ROOT.TH1F("h_mjj_pass" ,  "; mjj" ,     1000, 0., 2000. )
        self.addObject(self.h_mjj_tot)
        self.addObject(self.h_mjj_pass)
        # eff vs detajj
        self.h_detajj_tot  = ROOT.TH1F("h_detajj_tot" ,   "; detajj" ,     1000, 0., 10. )
        self.h_detajj_pass = ROOT.TH1F("h_detajj_pass" ,  "; detajj" ,     1000, 0., 10. )
        self.addObject(self.h_detajj_tot)
        self.addObject(self.h_detajj_pass)

        
        ### TTree
        self.out = ROOT.TTree("VBFtree","VBFtree")#wrappedOutputTree
        ...
        # pre-computed VBF quantities
        self.mjj    = np.empty((1), dtype="float32")#0
        self.pt1    = np.empty((1), dtype="float32")#0
        self.pt2    = np.empty((1), dtype="float32")#0
        self.eta1   = np.empty((1), dtype="float32")#0
        self.eta2   = np.empty((1), dtype="float32")#0
        self.phi1   = np.empty((1), dtype="float32")#0
        self.phi2   = np.empty((1), dtype="float32")#0
        self.detajj = np.empty((1), dtype="float32")#0
        self.out.Branch("mjj",    self.mjj    , "mjj/F"    )
        self.out.Branch("pt1",    self.pt1    , "pt1/F"    )
        self.out.Branch("pt2",    self.pt2    , "pt2/F"    )
        self.out.Branch("eta1",   self.eta1   , "eta1/F"   )
        self.out.Branch("eta2",   self.eta2   , "eta2/F"   )
        self.out.Branch("phi1",   self.phi1   , "phi1/F"   )
        self.out.Branch("phi2",   self.phi2   , "phi2/F"   )
        self.out.Branch("detajj", self.detajj , "detajj/F" )
        # --
        ...
        # offline selection flags
        self.passSelec_VBFincl = np.empty((1), dtype="float32")#0
        self.passSelec_VBFmu   = np.empty((1), dtype="float32")#0
        self.passSelec_VBFjets = np.empty((1), dtype="float32")#0
        self.passSelec_VBFmet  = np.empty((1), dtype="float32")#0
        self.passSelec_VBFphot = np.empty((1), dtype="float32")#0
        self.passSelec_VBFele  = np.empty((1), dtype="float32")#0
        self.passSelec_VBFtau  = np.empty((1), dtype="float32")#0
        self.out.Branch("passSelec_VBFincl", self.passSelec_VBFincl , "passSelec_VBFincl/F")
        self.out.Branch("passSelec_VBFjets", self.passSelec_VBFjets , "passSelec_VBFjets/F")
        self.out.Branch("passSelec_VBFmet" , self.passSelec_VBFmet  , "passSelec_VBFmet/F")
        self.out.Branch("passSelec_VBFmu"  , self.passSelec_VBFmu   , "passSelec_VBFmu/F")
        self.out.Branch("passSelec_VBFele" , self.passSelec_VBFele  , "passSelec_VBFele/F")
        self.out.Branch("passSelec_VBFphot", self.passSelec_VBFphot , "passSelec_VBFphot/F")
        self.out.Branch("passSelec_VBFtau" , self.passSelec_VBFtau  , "passSelec_VBFtau/F")
        # --
    ...
    def analyze(self, event):
        ...
        goodmuons = [m for m in muons if m.pt>30. and m.tightId and abs(m.eta)<2.4]
        refAccept = hlt_accept('IsoMu27') and len(goodmuons)>=1
        if not refAccept:
            return False
        ...
        ak4jets_VBF = [j for j in ak4jets if j.jetId==6 and j.pt > 50.]
        ...
        #TODO save trigger jets
        #hlt_jets = [obj for obj in trigobj if obj.id==1]
        
        threshold = thresholds('VBFincl')
        pT1min = threshold[0]+20.
        pT2min = threshold[1]+30.
        mjjmin = threshold[2]+200.
        detajjmin = threshold[3]

        pass_selection = False
        pass_selection_npv = False
        pass_selection_pt1 = False
        pass_selection_pt2 = False
        pass_selection_mjj = False
        pass_selection_detajj = False
        pt1vbf = -999
        pt2vbf = -999
        eta1vbf = -999
        eta2vbf = -999
        ...
        for idx1, p1 in enumerate(ak4jets_VBF):
            for idx2, p2 in enumerate(ak4jets_VBF):
                if idx1==idx2:
                    continue
                ...
                if mjj > mjjvbf:
                    ...
                    phi1vbf =p1.phi
                    phi2vbf = p2.phi
                    detajjvbf = abs(p1.eta - p2.eta)
                    idx_vbf1 = idx1
                    idx_vbf2 = idx2
        #if mjjvbf<0.:
        #    return False

        # selection for all plot is the same as "npv" selection,
        # just removing the cut on the displayed quantity
        pass_selection_npv = mjjvbf>=0 and (mjj > mjjmin) and\
                             (detajjvbf > detajjmin) and\
                             (pt2vbf > pT2min) and\
                             ((pt1vbf > pT2min and ak4jets[0].pt>pT1min) or\
                              (pt1vbf > pT1min))

        pass_selection_pt1 = mjjvbf>=0 and (mjj > mjjmin) and\
                             (detajjvbf > detajjmin) and\
                             (pt2vbf > pT2min)
        pass_selection_pt2 = mjjvbf>=0 and (mjj > mjjmin) and\
                             (detajjvbf > detajjmin) and\
                             ((pt1vbf > pT2min and ak4jets[0].pt>pT1min) or\
                              (pt1vbf > pT1min))
        pass_selection_mjj = mjjvbf>=0 and (detajjvbf > detajjmin) and\
                             (pt2vbf > pT2min) and\
                             ((pt1vbf > pT2min and ak4jets[0].pt>pT1min) or\
                              (pt1vbf > pT1min))
        pass_selection_detajj = mjjvbf>=0 and (mjj > mjjmin) and\
                                (pt2vbf > pT2min) and\
                                ((pt1vbf > pT2min and ak4jets[0].pt>pT1min) or\
                                 (pt1vbf > pT1min))  
        

        # trigger-dependant selection:
        # VBF inclusive: only VBF requirements
        # VBF + dijet: 2 additional jets with pT > X
        # etc
        pass_selec_VBFincl = True
        pass_selec_VBFmu   = True
        pass_selec_VBFjets = False
        pass_selec_VBFmet  = False
        pass_selec_VBFphot = False
        pass_selec_VBFele  = True # todo
        pass_selec_VBFtau  = True # todo

        pass_selec = True # Default histogramms stored, for inclusive path

        
        njet = 0
        for idx1, jet1 in enumerate(ak4jets):
            if idx1 == idx_vbf1 or idx1 == idx_vbf2:
                continue
            if jet1.pt > 70:
                njet +=1
            if njet > 1:
                pass_selec_VBFjets = True
                break

        
        if met.pt > 150.:
            pass_selec_VBFmet = True
        ...
        if 'VBFele' in triggerVersion: # ele pT > 20 GeV (safe enough?)
            #TODO
            print('blabla')
        if 'VBFtau' in triggerVersion: # tau pT > 50 GeV 
            #TODO
            print('blabla')
            
        #if not pass_selec:
        #    return False
        self.h_cutflow.Fill(3)
                
        self.MET_et[0]  = met.pt
        self.MET_phi[0] = met.phi

        #electrons = Collection(event,'Electrons')
        #taus      = Collection(event,'Taus')

        goodelectrons = [t for t in electrons if t.pt>20. and abs(t.eta)<2.4 and t.mvaIso_Fall17V2_WPL ]
        ...
        hltAccept = hlt_accept('VBF_DiPFJet105_40_Mjj1000_Detajj3p5') or\
                    hlt_accept('VBF_DiPFJet105_40_Mjj1000_Detajj3p5_TriplePFJet') 

                           
        if pass_selection_npv:
            self.h_npv_tot.Fill(PV.npvs)
            if hltAccept:
                self.h_npv_pass.Fill(PV.npvs)

        if pass_selection_mjj:
            self.h_mjj_tot.Fill(mjjvbf)
            if hltAccept:
                self.h_mjj_pass.Fill(mjjvbf)

        if pass_selection_pt1:
            self.h_pt1_tot.Fill(pt1vbf)
            if hltAccept:
                self.h_pt1_pass.Fill(pt1vbf)

        if pass_selection_pt2:
            self.h_pt2_tot.Fill(pt2vbf)
            if hltAccept:
                self.h_pt2_pass.Fill(pt2vbf)

        if pass_selection_detajj:
            self.h_detajj_tot.Fill(detajjvbf)
            if hltAccept:
                self.h_detajj_pass.Fill(detajjvbf)
        ...
        self.passVBFmet_old_2j[0]  = hlt_accept("DiJet110_35_Mjj650_PFMET110")
        self.passVBFmet_old_3j[0]  = hlt_accept("TripleJet110_35_35_Mjj650_PFMET110")
  
        self.passHHparking[0]     = hlt_accept("PFHT280_QuadPFJet30_PNet2BTagMean0p55")
        self.passHHparking_ref[0] = hlt_accept("PFHT280_QuadPFJet30")
        ...
        # etc
        self.passSelec_VBFincl[0] = pass_selec_VBFincl
        self.passSelec_VBFmu[0]   = pass_selec_VBFmu
        self.passSelec_VBFjets[0] = pass_selec_VBFjets
        self.passSelec_VBFmet[0]  = pass_selec_VBFmet
        self.passSelec_VBFphot[0] = pass_selec_VBFphot
        self.passSelec_VBFele[0]  = pass_selec_VBFele
        self.passSelec_VBFtau[0]  = pass_selec_VBFtau
        ...
        return True
        #return (accept and not pass_ref_trig) #accept
        #return pass_ref_trig
...
if __name__ == "__main__":
    ...
    parser.add_argument('--version', default='VBFincl',
                        choices = ['VBFincl',
                                   ...
                                   'VBFphot',
                                   'VBFtau'])
    parser.add_argument('--era', default = '23C',
                        choices = ['23C','23D','Winter23']) # more to be added
    # running on Muon0/1 for now (NanoAODv12); if when running on VBF dataset, will be found as `dataset=/ParkingVBF*/*/NANOAOD`
    #parser.add_argument('--input', default = '/store/data/Run2023C/Muon0/NANOAOD/PromptNanoAODv12_v2-v2/2820000/686646ef-4bf0-4cb7-a8da-f9cca5a5cc2f.root') # ~1.2M evts
    #parser.add_argument('--input', default = '/store/data/Run2023C/Muon0/NANOAOD/PromptNanoAODv11p9_v1-v1/2810000/0d7bec23-e475-4659-a7eb-ea8753f60b30.root')
    ...
    else: # no golden json (yet) for 23D
        p=PostProcessor(
            ".",
            files,
            cut=preselection,
